        ...
        filter.addEventListener("click", (event) => {
            ...
            if(url.match(matchRegExp)){
                ...
                if(!url.match(replaceRegExp)) {
                    window.location.replace(url.replace(new RegExp(`${abreviation},`), ""));
                } else {
                    window.location.replace(url.replace(replaceRegExp, ""));
                }
            ...
            if(url.match(/\/catalogue\?t=/)) {
                window.location.replace(url.concat(`${abreviation},`));
                return;
            }

            window.location.replace(url.concat(`?t=${abreviation},`));
        });
    };
});